...
#rmsnorm used over layer norm to reduce covariate shift, where re-scaling invariance is given importance as compared to re-centering invariance
#\ai = ai x gi/rms where gi is gamma parameter, one for each ai (check formula for proper understanding); rmsnorm computes one less statistic, so it is faster
class GemmaRMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        #llama does x.to(float16) but gemma is (x * w).to(float16)
...
class GemmaMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        #z = y * j [batch_size, seq_len, intermediate_size] (same dimension x same dimension for output matrix)
        #z = self.down_proj(z); [batch_size, seq_len, intermediate_size] => [batch_size, seq_len, hidden_size] (reverses the initial operation)
...
class GemmaModel(nn.Module):
    ...
    def forward(self, attention_mask : Optional[torch.Tensor] = None, 
        ...
        normaliser = torch.tensor(self.config.hidden_size**0.5, dtype=hidden_states.dtype)
